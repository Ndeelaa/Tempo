...
function customizationPanel() {
  ...
  const content={
    ...
    intensity:`<div class="creation-intensity"><p>À quel point ressens-tu cette émotion&nbsp;?</p><div role="radiogroup" aria-label="Intensité de l’émotion">${[1,2,3,4,5].map(level=>`<button role="radio" aria-checked="${state.intensity===level}" class="${state.intensity===level?'active':''}" data-intensity="${level}"><i style="--level:${level}"></i><span>${level}</span></button>`).join('')}</div><strong>${intensityNames[state.intensity]}</strong><small>Le rythme, la lumière et la présence de la forme s’adaptent immédiatement.</small></div>`,
